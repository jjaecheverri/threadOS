// Drifty — Bundled: openai
(function() {
'use strict';
// Drifty — Shared Content Script Base
...
function injectPanel() {
  ...
  Object.assign(toggle.style, {
    ...
    color: 'white',
    zIndex: '2147483641',
    boxShadow: '0 4px 16px rgba(184,146,106,.4)',
  ...
  toggle.addEventListener('click', () => {
    ...
    toggle.style.background = visible ? '#B8926A' : '#181818';
    toggle.style.border = visible ? 'none' : '1px solid #2a2a2a';
...
function injectSaveButton(anchorSel, getPrompt, getResponse) {
  ...
  const tryInject = () => {
    ...
    Object.assign(btn.style, {
      ...
      padding: '5px 12px',
      borderRadius: '7px',
      background: 'rgba(184,146,106,.1)',
      border: '1px solid rgba(184,146,106,.28)',
      color: '#B8926A',
      fontSize: '12px',
      fontWeight: '600',
    ...
    btn.onmouseenter = () => { btn.style.background = 'rgba(184,146,106,.2)'; };
    btn.onmouseleave = () => { btn.style.background = 'rgba(184,146,106,.1)'; };

    btn.onclick = async () => {
      const prompt = getPrompt();
      const response = getResponse();
      if (!prompt && !response) { flash(btn, 'Nothing yet', '#9A9490'); return; }
      flash(btn, 'Saving…', '#B8926A');
...
function flash(btn, text, color) {
  ...
  setTimeout(() => { btn.innerHTML = orig; btn.style.color = '#B8926A'; }, 2200);
}

function iconSVG(sz) {
  return `<svg width="${sz}" height="${sz}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0"><circle cx="12" cy="12" r="8"/><ellipse cx="12" cy="12" rx="8" ry="4" transform="rotate(-30 12 12)" opacity="0.7"/><circle cx="12" cy="4.5" r="1.5" fill="currentColor" stroke="none"/></svg>`;
}

// Drifty — ChatGPT (chat.openai.com) Content Script

let lastTurn = '';

const getUser = () => {
  const els = document.querySelectorAll('[data-message-author-role="user"] .whitespace-pre-wrap, [data-message-author-role="user"] [class*="whitespace"], article[data-testid*="user"] .whitespace-pre-wrap');
  return els[els.length - 1]?.textContent?.trim() || '';
};

const getAssistant = () => {
  const els = document.querySelectorAll('[data-message-author-role="assistant"] .markdown, [data-message-author-role="assistant"] [class*="markdown"], article[data-testid*="assistant"] .markdown');
  return els[els.length - 1]?.textContent?.trim() || '';
};
...
function init() {
  injectPanel();
  injectSaveButton('form .flex.w-full', getUser, getAssistant);
  startCapture();
  // Handle SPA navigation
  let url = location.href;
  new MutationObserver(() => {
    if (location.href !== url) { url = location.href; lastTurn = ''; setTimeout(init, 800); }
