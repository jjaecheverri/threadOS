...
// ThreadOS Sentinel — Perplexity Content Script
...
const USER_SELECTORS = [
  ...
  '[class*="UserQuery"]',
  '[class*="question"]',
  // Perplexity shows queries in specific containers
  'h2[class*="query"]',
  '[data-testid*="query"]',
...
const ASSISTANT_SELECTORS = [
  ...
  '[class*="response"]',
  '.prose',
  // Perplexity uses markdown for answers
  '[class*="markdown"]',
  '[data-testid*="answer"]',
];
...
function extractText(el) {
  if (!el) return '';
  const contentSels = ['.prose', '.markdown', 'p'];
    ...
    try {
      ...
      if (els.length) {
        ...
        if (text && text.length > 0) {
          if (captureCount < 5) console.log('[ThreadOS] ✓ User msg via:', sel);
          return text;
        }
      }
    } catch(e) {}
  ...
  for (const sel of ASSISTANT_SELECTORS) {
    try {
      ...
      if (els.length) {
        const text = extractText(els[els.length - 1]);
        if (text && text.length > 0) {
          if (captureCount < 5) console.log('[ThreadOS] ✓ Assistant msg via:', sel);
          return text;
        }
      }
    } catch(e) {}
...
function startCapture() {
  console.log('[ThreadOS] Perplexity capture v3 started');
  ...
  const diagInterval = setInterval(() => {
    ...
    const diag = {};
    ['[class*="query"]', '[class*="prose"]', '[class*="answer"]', '.break-words', 
     '[data-testid]', '[class*="markdown"]'].forEach(sel => {
...
function init() {
  console.log('[ThreadOS] Perplexity init v3');
  injectPanel();
