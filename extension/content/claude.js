...
// ThreadOS Sentinel — Claude.ai Content Script  
// Updated with robust multi-strategy selectors

let lastTurn = '';
let captureCount = 0;

// ── SELECTOR STRATEGIES ────────────────────────────────────────────────────
// Claude uses a conversation-turn structure with human/assistant roles
const USER_SELECTORS = [
  // Strategy 1: data-testid based (most stable on Claude)
  '[data-testid*="human"]',
  ...
  'div[class*="human"]',
  // Strategy 4: structural (Claude alternates human/assistant in the conversation)
  '.font-user-message',
  // Strategy 5: conversation turn containers
  '[class*="ConversationItem"]:has([class*="human"])',
  // Broadest fallback - human messages often use whitespace-pre-wrap
  '.whitespace-pre-wrap',
];
...
const ASSISTANT_SELECTORS = [
  ...
  'div[class*="assistant"]',
  // Strategy 4: Claude-specific
  '.font-claude-message',
  // Strategy 5: prose containers (Claude renders assistant messages in prose)
  '[class*="ConversationItem"] .prose',
  // Broadest fallback
  '.prose',
];
...
function extractText(el) {
  if (!el) return '';
  const contentSels = ['.prose', '.whitespace-pre-wrap', '.markdown', 'p'];
...
function tryCapture() {
  ...
  window.__tos_t = setTimeout(() => {
    ...
    lastTurn = finalKey;
    captureCount++;
    captureSession({ model: 'Claude', prompt: p, response: finalR });
  }, 2000);
}

function startCapture() {
  console.log('[ThreadOS] Claude capture v3 started — watching for messages...');
  ...
  const diagInterval = setInterval(() => {
    ...
    const diag = {};
    ['[data-testid]', '.prose', '.whitespace-pre-wrap', '[class*="human"]', '[class*="assistant"]', 
     '[data-testid*="human"]', '[data-testid*="assistant"]', '[class*="ConversationItem"]',
     '[data-role]'].forEach(sel => {
      try { diag[sel] = document.querySelectorAll(sel).length; } catch(e) { diag[sel] = 'err'; }
    });
...
function init() {
  console.log('[ThreadOS] Claude init v3');
  injectPanel();
  injectSaveButton('fieldset, form, [class*="composer"]', getUser, getAssistant);
  startCapture();
}
